...
const Join = () => {
  ...
  //메세지 영역이 각 입력창마다 있기 때문에 객체를 활용해 한 번에 관리
  ...
  //상태를 유지하려는 이유 -> 마지막에 회언 가입 버튼을 누를 때 까지 검증 상태를 유지해야 하기 때문.
  ...
  const saveInputState = ({ key, inputValue, msg, flag }) => {
    //입력값 세팅
    //패스워드 확인 입력값은 굳이  userVlaue상태로 유지할 필요가 없기 때문에
  ...
  //이름 입력창 체인지 이벤트 핸들러
  const nameHandler = (e) => {
    const nameRegex = /^[가-힣]{2,5}$/; //^,$정규 표현식의 시작과 끝
    const inputValue = e.target.value;

    //입력값 검증
    let msg; //검증 메제시를 저장할 변수
    ...
    // setMessage({...message,msg: msg});
    // setCorrect({...correct,flag: flag});

    //개게 프로퍼티에서 세팅하는 변수의 이름과 키값이 동일한 경우에는
    //콜론 생략이 가능
    saveInputState({
      key: 'userName',
      inputValue,
  ...
  const passwordHandler = (e) => {
    //패스워드가 변경됬었다 -> 패스워드 확인란을 비우고 시작하자.
    document.getElementById('password-check').value = ''; //랜더링이 된후 (이벤트 핸트러 안이라서)  가능한 문장.
